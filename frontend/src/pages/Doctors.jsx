    ...
    return (
        <div className="space-y-8 animate-fade-in pb-12">
            <div className="text-center max-w-2xl mx-auto space-y-4">
                <h1 className="text-5xl font-black text-slate-800 dark:text-neutral-100 tracking-tight transition-colors">{t('doctors.title')}</h1>
                <p className="text-slate-500 dark:text-neutral-400 font-medium text-lg leading-relaxed transition-colors">
                    Connect with top-rated medical professionals specialized in various fields.
                </p>
            </div>

            {/* Filter Section */}
            <div className="glass-card p-6 border-white/40 dark:border-white/5 bg-white/20 dark:bg-neutral-900/40 backdrop-blur-xl rounded-[2.5rem] shadow-2xl space-y-6 transition-colors">
            ...
            {loading ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {[1, 2, 3].map((n) => (
                        <div key={n} className="h-96 glass-card animate-pulse bg-white/30 dark:bg-neutral-900/40 rounded-[2.5rem] border-white/40 dark:border-white/5"></div>
                    ))}
                </div>
            ) : doctors.length === 0 ? (
                <div className="text-center py-20 glass-card bg-white/20 dark:bg-neutral-900/40 rounded-[2.5rem] border-white/40 dark:border-white/5">
                    <p className="text-2xl font-black text-slate-400 dark:text-neutral-500 uppercase tracking-widest">{t('doctors.noResults')}</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {doctors.map((doctor) => (
