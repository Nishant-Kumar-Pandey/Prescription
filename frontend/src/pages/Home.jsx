  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 py-8 transition-colors duration-300">
      <div className="text-center space-y-4 animate-fade-in">
        ...
        <p className="text-lg text-slate-500 dark:text-neutral-400 max-w-2xl mx-auto font-medium transition-colors">
          {t('home.subtitle')}
        </p>
      </div>
      ...
      {!result && !loading && (
        <div className="grid md:grid-cols-2 gap-8 items-stretch animate-fade-in">
          ...
          <div className="space-y-8 flex flex-col justify-center">
            <div className="glass-card p-8 border-l-4 border-l-medical-primary shadow-soft bg-white/50 dark:bg-neutral-900/40 dark:border-white/10">
              <h3 className="font-bold text-slate-800 dark:text-neutral-100 mb-4 text-lg">{t('home.howItWorks')}:</h3>
              <ul className="text-sm text-slate-600 dark:text-neutral-400 space-y-4">
            ...
            {history.map((item, i) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-[10px] text-slate-400 dark:text-neutral-500 font-bold transition-colors">{new Date(item.createdAt).toLocaleDateString()}</span>
                </div>
                <h4 className="font-bold text-slate-800 dark:text-neutral-100 truncate transition-colors">{item.medicines?.[0]?.name || "Prescription"}</h4>
                <p className="text-[11px] text-slate-500 dark:text-neutral-400 line-clamp-2 leading-relaxed transition-colors">
