...
const ResultCard = ({ result }) => {
    ...
    if (!result || !result.valid) {
        return (
            <div className="glass-card p-10 text-center space-y-4 max-w-2xl mx-auto border-red-200 dark:border-red-900/30 dark:bg-slate-900/40">
                ...
                <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">{t('results.incomplete')}</h3>
                <p className="text-slate-600 dark:text-slate-400 font-medium">{result?.reason || t('results.incomplete')}</p>
            </div>
        );
    }
        ...
        <div className="space-y-8 max-w-4xl mx-auto transition-colors duration-300">
            ...
            <div className="flex items-center gap-4 mb-8 bg-medical-primary/5 dark:bg-medical-primary/10 p-6 rounded-2xl border border-medical-primary/10 dark:border-white/5 relative overflow-hidden">
                ...
                <div className="flex-1">
                    <h2 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight transition-colors">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="glass-card p-6 border-l-4 border-l-medical-primary flex items-center space-x-4 bg-white/50 dark:bg-slate-900/40 dark:border-white/5">
                    ...
                    <div>
                        <p className="text-xs text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest">{t('results.medicinesDetected')}</p>
                        <h4 className="font-bold text-slate-800 dark:text-slate-100 transition-colors">{t('results.extractedFromRx')}</h4>
                    </div>
                </div>
                <div className="glass-card p-6 border-l-4 border-l-medical-secondary flex items-center space-x-4 bg-white/50 dark:bg-slate-900/40 dark:border-white/5">
                    ...
                    <div>
                        <p className="text-xs text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-widest">{t('results.aiConfidence')}</p>
                        <h4 className="font-bold text-slate-800 dark:text-slate-100 transition-colors">{t('results.highPrecision')}</h4>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-6">
                {result.medicines?.map((med, index) => (
                    <div key={index} className="glass-card p-8 space-y-6 relative overflow-hidden group hover:border-medical-primary/30 dark:hover:border-medical-primary/50 transition-all duration-300 bg-white/50 dark:bg-slate-900/40 dark:border-white/5">
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 relative z-10">
                            <div className="space-y-2">
                                <div className="flex items-center space-x-2">
                                    ...
                                    {med.route && <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase transition-colors">{med.route}</span>}
                                </div>
                                <h2 className="text-3xl font-black text-slate-800 dark:text-slate-100 tracking-tight transition-colors">{med.name}</h2>
                            ...
                            <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-slate-100 dark:border-white/5 px-5 py-3 rounded-2xl text-sm font-black text-slate-600 dark:text-slate-300 shadow-sm flex items-center space-x-2 transition-colors">
                        ...
                        <div className="grid md:grid-cols-5 gap-6 relative z-10">
                            <div className="md:col-span-3 space-y-4">
                                ...
                                <div className="bg-slate-50/70 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100/50 dark:border-white/5 transition-colors">
                                    <p className="text-slate-700 dark:text-slate-200 leading-relaxed font-semibold italic text-base">
                                        "{med.explanation}"
                                    </p>
                                </div>
                            </div>
            ...
            {/* General Advice & TTS */}
            <div className="glass-card p-10 space-y-8 border border-white/40 dark:border-white/10 shadow-2xl relative overflow-hidden bg-white/50 dark:bg-slate-900/40">
                ...
                {result.general_advice && (
                    <div className="space-y-4 relative z-10">
                        <h3 className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center">
                            <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            {t('results.generalAdvice')}
                        </h3>
                        <p className="text-slate-600 dark:text-slate-100 font-bold text-lg leading-relaxed bg-medical-primary/5 dark:bg-medical-primary/10 p-6 rounded-2xl border border-medical-primary/10 dark:border-white/5 transition-colors">
                            {result.general_advice}
                        </p>
                    </div>
                )}

                <div className="pt-6 border-t border-slate-100 dark:border-slate-800 space-y-8 relative z-10">
                    <AudioPlayer text={result.ttsText || t('results.noVoice')} />

                    {/* Technical Analysis Toggle (Requested feature) */}
                    <details className="group">
                        <summary className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] cursor-pointer hover:text-medical-primary transition-colors list-none flex items-center space-x-2">
